...
- 89 个 xunit 测试，约 4~5 秒完成。
- 已自动化覆盖：编译、单元逻辑、进程冷启动、单实例 + IPC、命名管道生命周期、`AppSettings` JSON round-trip、`HKCU\Run` 真实写入再清理、JSON 导出反向解析、隐私脱敏、`run.ps1` 全开关组合等。完整列表见 `doc/manual-test-plan.md` 附录 A。
- **必须人工确认**：托盘视觉、热插拔真硬件、Veto 真实分支、长稳定性 10 分钟监控。详见 `doc/manual-test-plan.md`。
...
卸载干净：退出程序 → 删除 `%LOCALAPPDATA%\UsbEjectHelper\` → `regedit` 删除 `HKCU\...\Run\UsbEjectHelper`（如果开过自启动）→ 删除 exe 所在目录。无服务、无驱动、无系统级残留。
...
- **MVP（已完成）** —— 托盘常驻、设备识别、安全弹出、双模式占用扫描、导出、自启动、单实例。
- **v0.2（设计中）** —— 管理员模式：用户主动以管理员重启时枚举更多句柄；"结束进程"二次确认流。
- **v0.3** —— 通知中心、典型场景 FAQ、可选代码签名（见 §11）。
...
**GPL v3 or later** — 全文见 [`UsbEjectHelper/LICENSE`](UsbEjectHelper/LICENSE)。
